...
app.get('/api/samp2', function (req, res) {
  const ip = req.query.ip;
  const port = req.query.port;
  const serverip = `${ip}:${port}`;
  const options = {
    ...
    .then((response) => {
      ...
      const responseJson = {
        'response': {
          ...
          'gamemode': response['raw']['gametype'],
          'language': response['raw']['mapname'],
          'passworded': response['password'],
          'maxplayers': response['maxplayers'],
          'ping': response['ping'],
          'isPlayerOnline': response['players'].length > 0,
          'rule': {
            'lagcomp': response['raw']['lagcomp'],
            'mapname': response['raw']['mapname'],
            'version': response['raw']['version'],
            'weather': response['raw']['weather'],
            'weburl': response['raw']['weburl'],
            'worldtime': response['raw']['worldtime']
          },
          'isPlayersIngame': playersList
        }
      };
